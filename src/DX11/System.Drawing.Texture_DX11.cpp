/*
 * System.Drawing.Texture_DX11.cpp
 *
 *  Created on: Taiwan
 *      Author: player
 */
...
#include "../../include/DX11/System.Drawing.Texture.h"
...
#pragma region class Texture1D
		Texture1D::Texture1D(Texture1D_Desc& desc, Renderer* pRender)
		{
			...
			if (SUCCEEDED(hr)) {
				...
				V(pDevice->CreateShaderResourceView(
					(ID3D11Resource *)_ptr,		//[in]   ID3D11Resource *pResource,
					&resourceViewDesc,			//[in]   const D3D11_SHADER_RESOURCE_VIEW_DESC *pDesc,
					&pResourceView				//[out]  ID3D11ShaderResourceView **ppSRView
					));
				if (SUCCEEDED(hr)) {
		...
		Texture1D::Texture1D(ID3D11Texture1D* pTexture, Renderer* pRender)
		{
			if (pTexture) {
				_ptr = pTexture;
		...
		Texture1D* Texture1D::FromFile(LPCTSTR pFileName, Renderer* pRender) {
			...
			if (SUCCEEDED(hr)) {
				ID3D11Texture1D* ptr = NULL;
				hr = pResource->QueryInterface(IID_ID3D11Texture1D, (LPVOID*)&ptr);
				pResource->Release();
				if (SUCCEEDED(hr)) {
...
#pragma region class Texture2D
		...
		Texture2D::Texture2D(ID3D11Texture2D* pTexture, Renderer* pRender)
		{
			if (pTexture) {
				_ptr = pTexture;
		...
		Texture2D* Texture2D::FromFile(LPCTSTR pFileName, Renderer* pRender) {
			...
			if (SUCCEEDED(hr)) {
				ID3D11Texture2D* ptr = NULL;
				hr = pResource->QueryInterface(IID_ID3D11Texture2D, (LPVOID*)&ptr);
				pResource->Release();
				if (SUCCEEDED(hr)) {
...
#pragma region class Texture3D
		...
		Texture3D::Texture3D(ID3D11Texture3D* pTexture, Renderer* pRender)
		{
			if (pTexture) {
				_ptr = pTexture;
		...
		Texture3D* Texture3D::FromFile(LPCTSTR pFileName, Renderer* pRender)
		{
			...
			if (SUCCEEDED(hr)) {
				ID3D11Texture3D* ptr = NULL;
				hr = pResource->QueryInterface(IID_ID3D11Texture3D, (LPVOID*)&ptr);
				pResource->Release();
				if (SUCCEEDED(hr)) {
